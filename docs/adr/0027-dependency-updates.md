...
Renovate は既定では `go.mod` の指定を書き換え、その版の `go.sum` エントリを足すところまでしかしない。書き換えの結果として要らなくなったエントリは残り、上流が新しいモジュールを経由し始めたときに要るようになったエントリは足されない。どちらも `go mod tidy` が直すものだが、Renovate はそれを回さない。

ADR 0028 は `tidy` の結果が反映されているかを CI で見ることにし、その代償として「Renovate の PR は `tidy` を回して結果を載せてからでないと通らない」ことを受け入れた。実際に届いた PR で分かったのは、落ちるのが `tidy` のジョブだけではないということである。`go.sum` が欠けた木では vet も lint も動かない。道具を固定した以上、道具を動かすにも `go.sum` が要る。

この一手間を人が払う理由はない。`postUpdateOptions` に `gomodTidy` を指定し、依存を書き換えた後の `tidy` を Renovate 自身に回させる。届く PR は `tidy` を通った状態になる。

ADR 0028 の決定は変わらない。CI は引き続きコミットされたものが `tidy` の結果と一致することを見るし、グラフが曖昧になれば `tidy` は差分を出す前に落ちる。変えたのは、その検査を通す作業を誰がやるかだけである。
...
Renovate の PR に人が `tidy` を回して結果を足す作業はなくなる。ADR 0028 が数えた代償のうち、この一手間の分は消える。

代わりに、`tidy` が失敗する場面での気づき方が変わる。グラフが再び曖昧になれば、まず Renovate 側で `tidy` が転び、PR が届かないか更新の反映されていない PR が届く。赤い CI ではなく Renovate のログが最初の手がかりになり、そのログはこのリポジトリの外にある。CI の検査は残っているので見落としはしないが、原因に近い情報は外に出た。

`tidy` はモジュールグラフ全体をダウンロードするため、Renovate が1つの更新に費やす時間は伸びる。こちらの待ち時間ではない。
...
- [[0028-keeping-the-module-graph-tidy]] — `tidy` の結果が反映されていることを CI で見る側。本 ADR の `gomodTidy` は、その検査を通す作業を Renovate に移したもの
